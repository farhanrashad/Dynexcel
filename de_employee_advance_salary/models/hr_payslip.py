...
class SalaryRuleInput(models.Model):
    ...
    def compute_sheet(self):
        for payslip in self:
            amount = 0 
            adv_salary = self.env['hr.salary.advance'].search([('employee_id', '=', payslip.employee_id.id), ('deductable', '=', True), ('state', 'in', ['paid','done'])])
            ...
            if adv_salary:
                for adv_obj in adv_salary:
                    ...
                    if current_date == existing_date:
                        ...
                        if not input_exists:
                            input_type_exists = self.env['hr.contract.advantage.template'].search([('code', '=', 'SAR')])            
                            input_exists.create({
