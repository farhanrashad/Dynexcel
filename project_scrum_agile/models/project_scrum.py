    ...
    def update_burndownchart(self):
        ...
        for sprint in self:
            ...
            for burn_ids in sprint.burndown_ids:
                burn_ids.unlink()
            date_start = \
                datetime.strptime(str(sprint.date_start),
                                  DEFAULT_SERVER_DATE_FORMAT)
            date_stop = \
                datetime.strptime(str(sprint.date_stop),
                                  DEFAULT_SERVER_DATE_FORMAT)
            ...
            for day in range(-1, total_days + 2):
                ...
                if date <= today:
                    for backlog in sprint.backlog_ids:
                        if (not str(backlog.date_done)) or (str(backlog. date_done) and str(backlog.date_done) < date):
                            sum_points += backlog.complexity
                        for task in backlog.tasks_id:
                            for work in task.timesheet_ids:
                                if str(work.date.date()) == date:
...
class ScrumMeeting(models.Model):
    ...
    def onchange_dates(self):
        ...
        start = datetime.strptime(str(self.start_datetime),
                                  DEFAULT_SERVER_DATETIME_FORMAT)
            ...
            for req in self:
                ...
                for partner_ids in req.message_follower_ids:
                    if user.partner_id.id != partner_ids.partner_id.id:
                        self.env['mail.followers']._insert_followers(self._name,
                                                                     [req.id],
                                                                     [user.partner_id.id],
                                                                     None,
                                                                     {},
